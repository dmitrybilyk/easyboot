package com.learn.easyboot.bases.patterns.behavioral.strategy;
...
import static com.learn.easyboot.bases.patterns.behavioral.strategy.StrategyPatternCheck2.studentsDBHolder;
...
public class StrategyPatternCheck2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(studentFinder.findStudents(StudentLookup.builder().name("a").age(30).build()));
    }
}
...
class StrategyByNameImpl implements Strategy {
    ...
    public List<Student> execute(StudentLookup studentLookup) {
        return studentsDBHolder.getStudentList().stream().filter(student ->
                student.getName().contains(studentLookup.getName())).collect(Collectors.toList());
    }
}
...
class StrategyByAgeImpl implements Strategy {
    ...
    public List<Student> execute(StudentLookup studentLookup) {
        return studentsDBHolder.getStudentList().stream().filter(student ->
                student.getAge() == studentLookup.getAge()).collect(Collectors.toList());
    }
}
